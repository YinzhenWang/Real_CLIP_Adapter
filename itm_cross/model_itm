...
class CLIPAll_SimpleCrossAttn(nn.Module):
    ...
    def forward(self, input_ids, pixel_values, attention_mask, image_id=None):
        ...
        # Normalization, compute similarity
        uni_vision_embeds = uni_vision_embeds / uni_vision_embeds.norm(p=2, dim=-1, keepdim=True)
        uni_text_embeds = uni_text_embeds / uni_text_embeds.norm(p=2, dim=-1, keepdim=True)

        cliploss, logits_per_image, logits_per_text = self.criterion(uni_vision_embeds, uni_text_embeds, self.logit_scale.exp())

        weights_i2t = F.softmax(logits_per_image[:,:batch_size]+1e-4,dim=1)
        weights_t2i = F.softmax(logits_per_text[:,:batch_size]+1e-4,dim=1)
        idx = torch.arange(batch_size).to(input_ids.device)
        idx = idx.view(-1,1)
        mask = torch.eq(idx, idx.T).to(input_ids.device)
        ...
        image_embeds_neg = [] 
        image_pe_neg = []
        input_ids_neg_vision = []
        for b in range(batch_size):
            neg_idx = torch.multinomial(weights_t2i[b], 1).item()
            image_embeds_neg.append(unimodal_vision_outputs[neg_idx])
            image_pe_neg.append(image_pe[neg_idx])
            input_ids_neg_vision.append(input_ids[neg_idx])
        image_neg = torch.stack(image_embeds_neg,dim=0)  
        image_pe_neg = torch.stack(image_pe_neg,dim=0) 
        input_ids_neg_vision = torch.stack(input_ids_neg_vision,dim=0) 
        ...
        for b in range(batch_size):
            ...
            text_embeds_neg.append(unimodal_text_outputs[neg_idx])
            text_pe_neg.append(text_pe[neg_idx])
            input_ids_neg_text.append(input_ids[neg_idx])
        text_neg = torch.stack(text_embeds_neg,dim=0)
        text_pe_neg = torch.stack(text_pe_neg,dim=0)
        input_ids_neg_text = torch.stack(input_ids_neg_text,dim=0) 
